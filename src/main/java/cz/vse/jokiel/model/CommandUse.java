...
public class CommandUse implements ICommand
{
     ...
     * @param game hra
     */
    public CommandUse(GamePlan plan, Game game)
    {
        this.plan = plan;
        this.game = game;
    }
    ...
    private String pocitac()
    {
        if(plan.getCurrentArea().getName().equals("Policejni_stanice"))
        {
            Computer pocitac = new Computer(plan);
            System.out.println(pocitac.start());
    ...
    private String automatNaKafe()
    {
        if(plan.getCurrentArea().getName().equals("Policejni_stanice"))
        {
            if(plan.getBackpack().howManyItemsInStorage() < 5)
            {
                ...
                {  
                    System.out.println("Použil jsi Prazdny_kelimek");
                    plan.getBackpack().removeItem(plan.getBackpack().getItem("Prazdny_kelimek"));
                }
                System.out.println("Please stand by. Your coffee is being made.");
                try{waitSec(2000);}catch(InterruptedException e){System.out.println("");}
    ...
    private String pouta()
    {
        if(plan.getBackpack().isInStorage(itemName))
        {
            if(plan.getCurrentArea().getName().equals("Skrys"))
            {
                ...
                System.out.println(plan.getNPC("Pachatel").getStringDialog(1));
